...
export function uintToHex(value: number, byteLen = 1): string {
  return value.toString(16).padStart(byteLen * 2, '0');
}

/** Decode a hex string to a signed integer (two's complement) */
export function hexToInt(hex: string, byteLen = 2): number {
...
export function hexToBool(hex: string): boolean {
  return hexToUint(hex) !== 0;
}

/** Encode a boolean to a hex byte */
export function boolToHex(val: boolean): string {
  return val ? '01' : '00';
}
 ...
 */
export function erdTempToFahrenheit(hex: string): number {
  // Temperatures are usually 2 bytes, big-endian, signed
  return hexToInt(hex, 2);
}

export function fahrenheitToErd(tempF: number): string {
...
export function erdTimeToSeconds(hex: string): number {
  if (!hex || hex === '000000' || hex === '0000') return 0;
  if (hex.length <= 4) {
    // 2-byte: HHMM
...
export function secondsToErdTime(totalSeconds: number, byteLen = 3): string {
  const h = Math.floor(totalSeconds / 3600);
  const m = Math.floor((totalSeconds % 3600) / 60);
  const s = totalSeconds % 60;
  if (byteLen === 2) return `${uintToHex(h)}${uintToHex(m)}`;
  return `${uintToHex(h)}${uintToHex(m)}${uintToHex(s)}`;
}

// ─── Bitmask helpers ──────────────────────────────────────────────────────────

export function getBit(hex: string, bit: number): boolean {
  return (hexToUint(hex) & (1 << bit)) !== 0;
}

export function setBit(hex: string, bit: number, val: boolean, byteLen = 1): string {
  let n = hexToUint(hex);
  if (val) n |= (1 << bit);
  else     n &= ~(1 << bit);
 ...
 */
export function decodeDoorStatus(hex: string): { fridge: boolean; freezer: boolean; drawer: boolean } {
 ...
 */
export function decodeLaundryTimeRemaining(hex: string): number {
 ...
 */
export function decodeAcTemp(hex: string): number {
  const raw = hexToUint(hex);
  // Raw >= 60: treat as absolute °F
  return raw >= 60 ? raw : raw + 60;
}

export function encodeAcTemp(tempF: number): string {
